<?php
	...
	public  $con; 
		...
		public function InsertNewSchedule($post)
		{
			...
			$AverageSessionTime = $this->con->real_escape_string($_POST['AverageSessionTime']);
			if(empty($DoctorsScheduleday)||empty($DoctorsScheduledate||empty($DoctorsScheduleEnd)||empty($DoctorsScheduleStart)||empty($AverageSessionTime)))
			{
                header("Location:index.php?msg4=missing");
			}
			else
			{
			if($DoctorsScheduleStart>=$DoctorsScheduleEnd)
			{
				header("Location:index.php?msg5=Valid");
			}
else{
			...
			if ($result) {
header("Location:index.php?msg2=insert");
} else {
